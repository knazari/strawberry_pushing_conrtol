...
class RobotReader(object):
    def __init__(self,save_path):
        ...
        while raw_input("press enter to start saving data, or type ctrl c then n to not: ") != "n":
            ...
            self.haptic_finger_sub = rospy.Subscriber('/fing_camera/color/image_raw', Image, self.finger_callback)
            self.robot_state
            subscribers = [self.robot_state, self.haptic_finger_sub]#, self.xela_sub, self.prox_sub]
    ...
    def read_robot_data(self, robot_joint_data, haptic_finger_data):
    ...
    def save_data(self):
        ...
        folder = str('./data_sample_' + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
