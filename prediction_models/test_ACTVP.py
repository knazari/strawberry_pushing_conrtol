...
class Model(nn.Module):
    def __init__(self, features):
        ...
        self.convlstm1 = ConvLSTMCell(input_dim=524, hidden_dim=524, kernel_size=(3, 3), bias=True, device=self.device)
        self.convlstm2 = ConvLSTMCell(input_dim=524, hidden_dim=524, kernel_size=(3, 3), bias=True, device=self.device)
        ...
        self.upconv1   = nn.Conv2d(in_channels=524, out_channels=256, kernel_size=5, stride=1, padding=2)
        # self.upconv2 = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=5, stride=1, padding=2)
    ...
    def run(self, scene, actions, touch, test=True):
        mae = 0
...
pred_model.load_state_dict(torch.load("/home/kiyanoush/Downloads/TactileEnabledModel/2DPushing_500_Epochs/model_27_01_2023_15_33/ATCVP_model", map_location='cpu'))
pred_model.eval()


touch_dummy_input = torch.randn(10, 2, 3, 256, 256)
action_dummy_input = torch.randn(20, 2, 6)
scene_dummy_input = torch.randn(10, 2, 3, 256, 256)

# print(pred_model)
import time

start = time.time()
pred_model.run(touch_dummy_input, action_dummy_input, scene_dummy_input, test=True)
