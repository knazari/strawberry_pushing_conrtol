...
##### FOR FIGURE 4 IN THE PAPER (SINGLE STRAWBERRY RESULTS) USE: TIP: REACTIVE 005, PROACTIVE: 002, AND OPENLOOP: 013
#####                                        BASE: REACTIVE 007, PROACTIVE: 005, AND OPENLOOP: 015
...
open_loop_stem_pose = np.load(data_path + "/reactive" + "/013" + "/localisation.npy")

reactive_robot_pose = np.load(data_path + "/reactive/final_tests" + "/005" +  "/robot_pose.npy")
reactive_robot_vel  = np.load(data_path + "/reactive/final_tests" + "/005" +  "/robot_velocity.npy")
reactive_stem_pose  = np.load(data_path + "/reactive/final_tests" + "/005" +  "/localisation.npy")

proactive_robot_pose = np.load(data_path + "/proactive/final_tests" + "/002" +  "/robot_pose.npy")
proactive_robot_vel  = np.load(data_path + "/proactive/final_tests" + "/002" +  "/robot_velocity.npy")
proactive_stem_pose  = np.load(data_path + "/proactive/final_tests" + "/002" +  "/localisation_offline.npy")
...
ax.plot(contact_initialised/60 + np.arange(len(reactive_robot_vel[contact_initialised:]))/60, reactive_stem_pose[contact_initialised:], c='b', linewidth=2)
ax.plot(contact_initialised/60 + np.arange(len(proactive_robot_vel[contact_initialised:]))/60, proactive_stem_pose[contact_initialised:]-0.1, c='r', linewidth=2)
...
######################################################################
########################### SENSOR BASE ##############################
open_loop_stem_pose = np.load(data_path + "/reactive" + "/015" + "/localisation.npy")

reactive_robot_pose = np.load(data_path + "/reactive/final_tests" + "/006" +  "/robot_pose.npy")
reactive_robot_vel  = np.load(data_path + "/reactive/final_tests" + "/006" +  "/robot_velocity.npy")
reactive_stem_pose  = np.load(data_path + "/reactive/final_tests" + "/006" +  "/localisation.npy")

proactive_robot_pose = np.load(data_path + "/proactive/final_tests" + "/005" +  "/robot_pose.npy")
proactive_robot_vel  = np.load(data_path + "/proactive/final_tests" + "/005" +  "/robot_velocity.npy")
proactive_stem_pose  = np.load(data_path + "/proactive/final_tests" + "/005" +  "/localisation_offline.npy")
...
ax.plot(np.arange(len(reactive_robot_vel[:contact_initialised+1]))/60, np.zeros(len(reactive_robot_vel[:contact_initialised+1])), c='b', linewidth=2, alpha=0.05)
ax.plot(contact_initialised/60 + np.arange(len(open_loop_stem_pose[contact_initialised:]))/60, open_loop_stem_pose[contact_initialised:]-0.4, c='g', linewidth=2)
ax.plot(contact_initialised/60 + np.arange(len(reactive_robot_vel[contact_initialised:]))/60, reactive_stem_pose[contact_initialised:], c='b', linewidth=2)
ax.plot(contact_initialised/60 + np.arange(len(proactive_robot_vel[contact_initialised:]))/60, proactive_stem_pose[contact_initialised:]-1.4, c='r', linewidth=2)
...
ax.text(contact_initialised/60 - 0.08, -0.35, "Contact initiated", rotation='vertical', fontsize=14)
...
ax.arrow(0.05, 0.03, 0.0, 0.1, width= 0.02, head_length=0.04, color='black')
ax.arrow(0.05, -0.04, 0.0, -0.1, width= 0.02, head_length=0.04, color='black')
ax.text(0.018, 0.23, "Sensor tip", rotation='vertical', fontsize=14)
ax.text(0.011, -0.85, "Sensor base", rotation='vertical', fontsize=14)
...
ax.legend(custom_lines, ['Open-loop', 'PD-tactile', 'd-FPC'], fontsize=11, loc="lower right", bbox_to_anchor=(0.4,0.75))
plt.savefig("/home/kiyanoush/Cpp_ws/src/haptic_finger_control/RT-Data/stem_pose.png")
