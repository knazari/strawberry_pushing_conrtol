...
class Model(nn.Module):
    ...
    def forward(self, scene, actions, touch, test=False):
        ...
        for index, (sample_scene, sample_action, sample_touch) in enumerate(zip(scene[0:-1], actions[1:], cycle(touch[:-1]))):
            if index > self.context_frames - 1:
                ...
                # LSTM Chain
                hidden_1, cell_1 = self.convlstm1(input_tensor=robot_and_touch.double(),
                ...
                skip_connection_added = torch.cat((up4, output.double()), 1)
                ...
                # LSTM Chain
                #
                hidden_1, cell_1 = self.convlstm1(input_tensor=robot_and_touch.double(), cur_state=[hidden_1, cell_1])
                hidden_2, cell_2 = self.convlstm2(input_tensor=hidden_1, cur_state=[hidden_2, cell_2])
                ...
                #
                skip_connection_added = torch.cat((up4, sample_touch.view(1, 3, 256, 256).double()), 1)
                output = self.conv2(skip_connection_added)
